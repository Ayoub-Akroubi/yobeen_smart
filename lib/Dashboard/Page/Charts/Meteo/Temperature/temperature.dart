...
class TemperatureChart extends StatefulWidget {
  dynamic type;
  TemperatureChart({Key? key, required this.type}) : super(key: key);
...
class _TemperatureChartState extends State<TemperatureChart> {
  ...
  dynamic dataChart = [];
  List types = ['daily', 'hourly', 'brute', 'monthly'];
  List listOfSlug = [];
            ...
            setState(() {
              for (var i = 0; i < value.length; i++) {
                if (types[j] == 'daily') {
                  ...
                  dataChartDaily.add([timestamp1, value[i]['T']]);
                } else if (types[j] == 'hourly') {
                  final datetimevalue = DateTime.parse(value[i]['datetime']);
                  final timestamp1 = datetimevalue.millisecondsSinceEpoch;
                  dataChartHourly.add([timestamp1, value[i]['T']]);
                } else if (types[j] == 'brute') {
                  final datetimevalue = DateTime.parse(value[i]['datetime']);
                  final timestamp1 = datetimevalue.millisecondsSinceEpoch;
                  dataChartBrute.add([timestamp1, value[i]['T']]);
                } else if (types[j] == 'monthly') {
                  final datetimevalue = DateTime.parse(value[i]['datetime']);
                  final timestamp1 = datetimevalue.millisecondsSinceEpoch;
                  dataChartMonthly.add([timestamp1, value[i]['T']]);
                }
              }
            })
          });
